...
async def login(
    ...
    jwt_pair: dict = await user_service.login_by_credentials(
        username=form_data.username, password=form_data.password, request=request
    )
...
async def logout_other_devices(user_service: UserService = Depends(get_user_service)):
    """
...
async def forgot_password(
    ...
    reset_token = reset_password_service.create_reset_token(user.id, user.email)
    await reset_password_service.send_reset_password_email(request.email, reset_token)
    await reset_password_service.increase_requests(request.email)

    return {"message": "We have sent an email to you. Check your mailbox please."}
...
async def get_public_key(
    ...
    return {"public_key": auth_service.public_key}

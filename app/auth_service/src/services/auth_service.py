...
class AuthService:
    ...
    def create_access_token(self, to_encode: dict, token_version: datetime) -> str:
        """
    ...
    async def update_token_blacklist(self, token_blacklist) -> None:
        payload = jwt.decode(token_blacklist, self.public_key, algorithms=["RS256"])
    ...
    async def refresh_tokens_for_user(self, user: User) -> dict:
        ...
        access_token = self.create_access_token(
            to_encode=access_token_data, token_version=user.token_version
        )
    ...
    async def get_or_create_oauth_user(
        self, provider: str, user_info: dict
    ) -> User | None:
        """
        ...
        user = await self.user_repository.get_by_yandex_id(user_info["id"])
        if user:
            return user
    ...
    async def check_unique_username(self, username) -> bool:
        ...
        return True if self.user_repository.get_by_username(username) is None else False

...
class ElasticsearchLoader:
    def __init__(self, host: str, port: int):
        ...
        def wait_for_es():
            ...
            if not self.es.indices.exists(index="genres"):
                self.es.indices.create(index="genres", mappings=esis.genres_mappings)
            if not self.es.indices.exists(index="persons"):
                self.es.indices.create(index="persons", body=self.index_persons)
    ...
    def load_genres(self, genres: list[dict[str, Any]]):
        ...
        for genre in genres:
            try:
                sanitized_genre = {"id": genre["id"], "name": genre.get("name", "")}
                sanitized_genres.append(sanitized_genre)
            except Exception as e:
                logger.error(f"Error sanitizing movie: {genre}, error: {e}")
        actions = [
            {"_index": "genres", "_id": genre["id"], "_source": genre}
                        ...
                        if value is None:
                            if key == "imdb_rating":
                                ...
                                sanitized_movie[key] = (
                                    ...
                                    or key
                                    in ["genres", "actors", "directors", "writers"]
                                    else ""
                                )
        ...
        for page in pages:
            for person in page:
                try:
                    ...
                    logger.error(f"Error sanitizing movie: {person}, error: {e}")

...
class FilmService(AbstractService):
    ...
    def __init__(
        self,
        cache_service: AbstractFilmCacheService,
    ...
    async def get_by_id(self, film_id: str) -> MovieDetailResponse | None:
        ...
        if not film:
            ...
            film = await self.search_platform.get_film_from_search_platform(film_id)
            if not film:
                return None
    ...
    async def get_similar_by_id(
        ...
        if not similar_films:
            ...
            similar_films = await self.search_platform.search_film_in_search_platform(
                page_number=page_number, page_size=page_size, sort=sort, genre=genre_id
            )
            if not similar_films:
                return None

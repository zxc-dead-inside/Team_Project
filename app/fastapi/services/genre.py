...
class GenreService(AbstractService):
    ...
    def __init__(
        self,
        cache_service: AbstractGenreCacheService,
    ...
    async def search_query(
        self, page_number: int, page_size: int, sort: str | None = None
    ) -> list[Genre] | None:
    ...
    async def get_by_id(self, genre_id: str) -> Genre | None:
        ...
        if not genre:
            genre = await self.search_platform.get_genre_from_search_platform(genre_id)
            if not genre:
                return None

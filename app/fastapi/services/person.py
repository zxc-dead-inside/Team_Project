...
class PersonService(AbstractService):
    ...
    def __init__(
        self,
        cache_service: AbstractPersonCacheService,
    ...
    async def search_query(
        ...
        if not persons:
            persons = await self.search_platform.search_person_in_search_platform(
    ...
    async def get_by_id(self, person_id: str) -> Person | None:
        ...
        if not person:
            person = await self.search_platform.get_person_from_search_platform(
                person_id
            )

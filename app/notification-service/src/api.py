...
from src.services import (
    MessageTemplateService, MessageService, ScheduledTaskService
)
...
def create_template(
    template: MessageTemplateCreate, db: Session = Depends(get_db)
):
...
def update_template(
    ...
    updated_template = MessageTemplateService.update_template(
        db, template_id, template
    )
...
def create_scheduled_task(
    task: ScheduledTaskCreate, db: Session = Depends(get_db)
):
...
def get_scheduled_task(task_id: int, db: Session = Depends(get_db)):
    ...
    if task is None:
        raise HTTPException(
            status_code=404, detail="Запланированная задача не найдена"
        )
    return task
...
def update_scheduled_task(
    ...
    updated_task = ScheduledTaskService.update_scheduled_task(
        db, task_id, task
    )

...
Перейдите в каталог `infra-etl`, переименуйте файл `.env_example` в `.env` и выполните команду

```shell
docker compose up -d --build
```

Обратите внимание, что для создания базы postgresql берется дамп из корневого каталога `infra_theatre/db`, а каталог с файлами базы создается в подкаталоге `infra_etl/db/data`. Сделано это для того, чтобы экземпляры базы были созданы одинаковые, но не пересекались при запуске etl процесса и всего проекта.

По умолчанию на самом хосте ES доступен на порту 9201. Если запускать одновременно с контейнерами всего проекта будет конфликт с портами!
Если порты заняты исправьте на нужные в `docker-compose.yml`.
